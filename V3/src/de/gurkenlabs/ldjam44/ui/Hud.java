...
public class Hud extends GuiComponent {
	...
	private final AnimationController useButtonAnimationController = new AnimationController(
			Resources.spritesheets().get("hud-use-button"));
	...
	public void render(Graphics2D g) {
		...
		if (Game.world().environment() == null || Player.instance().isDead()
				|| Player.instance().getState() != PlayerState.CONTROLLABLE
				|| GameManager.getState() != GameState.INGAME) {
			return;
		}
	...
	private void renderKeeperUI(Graphics2D g) {
		...
		ImageRenderer.render(g, arrow, (loc.getX() * Game.world().camera().getRenderScale() - arrow.getWidth() / 2.0),
				loc.getY() * Game.world().camera().getRenderScale() - (arrow.getHeight() * 2.5));
		...
		for (Enemy enemy : Game.world().environment().getByType(Enemy.class)) {
			if (enemy.isEngaged() && !enemy.isDead()) {
				...
				final double currentWidth = width
						* (enemy.getHitPoints().getCurrentValue() / (double) enemy.getHitPoints().getMaxValue());
			...
			if (!enemy.isEngaged() && !enemy.isEngaging()) {
				...
				ImageRenderer.render(g, arrow,
						(loc.getX() * Game.world().camera().getRenderScale() - arrow.getWidth() / 2.0),
						loc.getY() * Game.world().camera().getRenderScale() - (arrow.getHeight() * 2.5));
			}
		}
	}
	...
	private void renderSlaves(Graphics2D g) {
		...
		TextRenderer.renderWithOutline(g, "x" + GameManager.getOwnSlaveCount(), x, y + SLAVE.getHeight(), Color.BLACK,
	...
	private void renderHP(Graphics2D g) {
		...
		double x = Game.window().getResolution().getWidth() / 2.0
				- ((Player.instance().getHitPoints().getMaxValue() * (HEART.getWidth() + PADDING) * 0.5) - PADDING);
	...
	private void renderUseButton(Graphics2D g) {
		if (Player.instance().getState() == PlayerState.CONTROLLABLE
				&& (Player.instance().canTrigger() || Player.instance().canTalkToGateKeeper())) {
			...
			ImageRenderer.render(g, useButton,
					(loc.getX() * Game.world().camera().getRenderScale() - useButton.getWidth() / 2.0),
					loc.getY() * Game.world().camera().getRenderScale() - (useButton.getHeight() * 2.5));
		}
	}
}
