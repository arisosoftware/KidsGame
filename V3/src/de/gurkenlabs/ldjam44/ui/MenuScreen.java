...
public class MenuScreen extends Screen implements IUpdateable {
	...
	public static final BufferedImage CONTROLS = Imaging.scale(Resources.images().get("controls.png"),
			(int) (Game.window().getResolution().getWidth() / 4.0),
			(int) (Game.window().getResolution().getHeight() / 2.0), true);
	// private static final BufferedImage BG =
	// Imaging.scale(Resources.images().get("menu_bg.png"),
	...
	protected void initializeComponents() {
		...
		this.mainMenu = new KeyboardMenu(centerX - buttonWidth / 2, centerY * 1.3, buttonWidth, centerY / 2, "Play",
				"Instructions", "Exit");
	...
	public void render(final Graphics2D g) {
		...
		TextRenderer.renderWithOutline(g, COPYRIGHT, centerX - stringWidth / 2,
				Game.window().getResolution().getHeight() * 19 / 20, Color.BLACK,
	...
	private void renderScrollingStuff(Graphics2D g) {
		ImageRenderer.render(g, CLOUD1,
				-CLOUD1.getWidth()
						+ Game.time().now() * 0.1 % (CLOUD1.getWidth() + Game.window().getResolution().getWidth())
						+ cloud1Offset,
				cloud1Offset);
		ImageRenderer.render(g, CLOUD2,
				-CLOUD2.getWidth()
						+ Game.time().now() * 0.2 % (CLOUD2.getWidth() + Game.window().getResolution().getWidth())
						+ cloud2Offset,
				cloud2Offset);
		ImageRenderer.render(g, CLOUD3,
				-CLOUD3.getWidth()
						+ Game.time().now() * 0.3 % (CLOUD3.getWidth() + Game.window().getResolution().getWidth())
						+ cloud3Offset,
				cloud3Offset);
		ImageRenderer.render(g, CLOUD4,
				-CLOUD4.getWidth()
						+ Game.time().now() * 0.4 % (CLOUD4.getWidth() + Game.window().getResolution().getWidth())
						+ cloud4Offset,
				cloud4Offset);
		ImageRenderer.render(g, Imaging.horizontalFlip(CLOUD1),
				-CLOUD1.getWidth()
						+ Game.time().now() * 0.5 % (CLOUD1.getWidth() + Game.window().getResolution().getWidth())
						+ cloud1Offset,
				cloud1Offset);
		ImageRenderer.render(g, Imaging.horizontalFlip(CLOUD2),
				-CLOUD2.getWidth()
						+ Game.time().now() * 0.6 % (CLOUD2.getWidth() + Game.window().getResolution().getWidth())
						+ cloud2Offset,
				cloud1Offset);
		ImageRenderer.render(g, Imaging.horizontalFlip(CLOUD3),
				-CLOUD3.getWidth()
						+ Game.time().now() * 0.7 % (CLOUD3.getWidth() + Game.window().getResolution().getWidth())
						+ cloud3Offset,
				cloud2Offset);
		ImageRenderer.render(g, Imaging.horizontalFlip(CLOUD4),
				-CLOUD4.getWidth()
						+ Game.time().now() * 0.8 % (CLOUD4.getWidth() + Game.window().getResolution().getWidth())
						+ cloud4Offset,
				cloud4Offset);
		ImageRenderer.render(g, STATUE,
				Game.window().getResolution().getWidth()
						- Game.time().now() % (STATUE.getWidth() + Game.window().getResolution().getWidth()),
				Game.window().getResolution().getHeight() - STATUE.getHeight());
