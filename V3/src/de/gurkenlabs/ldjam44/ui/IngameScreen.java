...
//TODO:Fix sound resource issus  public static final Sound INGAME_MUSIC = Resources.sounds().get("ingamemusic.ogg");
	...
	protected void initializeComponents() {
		...
		ingameMenu = new KeyboardMenu(centerX - buttonWidth / 2, centerY * 1.3, buttonWidth, centerY / 2, "Continue",
				"Exit");
		...
		deathMenu = new KeyboardMenu(centerX - buttonWidth / 2, centerY * 1.3, buttonWidth, centerY / 2, "Retry",
				"Exit");
	...
	public void render(Graphics2D g) {
		if (Player.instance().getState() == PlayerState.LOCKED) {
			g.setClip(new Rectangle2D.Double(0, CINEMATIC_BORDER, Game.window().getResolution().getWidth(),
					Game.window().getResolution().getHeight() - CINEMATIC_BORDER * 2));
		...
		deathMenu.setVisible(GameManager.getState() == GameState.INGAME && Player.instance().isDead()
				|| GameManager.getState() == GameState.SLAVES_DEAD);

		if (GameManager.getState() == GameState.INGAME_MENU) {
			g.setColor(new Color(0, 0, 0, 100));
			g.fillRect(0, 0, (int) Game.window().getResolution().getWidth(),
					(int) Game.window().getResolution().getHeight());
		...
		if (Game.world().environment() != null && levelNameTick != 0 && GameManager.getState() == GameState.INGAME) {
			...
			if (deltaTime > 1000 && deltaTime < LEVELNAME_DURATION) {
				...
				TextRenderer.renderWithOutline(g, cityName, x, 150, new Color(0, 0, 0, alpha),
				...
				if (description != null && !description.isEmpty()) {
					...
					TextRenderer.renderWithOutline(g, description, x2, 210, new Color(0, 0, 0, alpha),
							RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
					g.setFont(old);
				}
			}
	...
	public void update() {
		if (this.lastPlayed == 0) {
			// TODO:Fix sound resource issus Game.audio().playMusic(INGAME_MUSIC);
