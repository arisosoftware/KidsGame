...
public class ChargeEffect extends Effect {
	...
	public void update() {
		...
		final double maxPixelsPerTick = this.getAbility().getAttributes().getValue().getCurrentValue() / 1000.0
				* Math.min(deltaTime, 50);

		if (!wasHit) {
			for (ICombatEntity ent : Game.world().environment().findCombatEntities(
					this.getAbility().getExecutor().getHitBox(), e -> e.equals(Player.instance()))) {
