...
public class HitEmitter extends Emitter {
	...
	private static final Color[] DEFAULT_HIT_COLORS = new Color[] { new Color(104, 23, 40), new Color(146, 25, 42),
			new Color(153, 39, 34) };
	...
	public Particle createNewParticle() {
		...
		return new RectangleFillParticle(size, size, color, life).setCollisionType(Collision.ANY).setDeltaX(dx)
				.setDeltaY(dy).setDeltaIncX(gravityX).setDeltaIncY(gravityY);
	}
