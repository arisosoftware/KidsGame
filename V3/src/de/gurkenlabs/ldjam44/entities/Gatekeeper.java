...
public class Gatekeeper extends Creature {
	...
	public Gatekeeper() {
		...
		this.addMessageListener(l -> {
			...
			if (l.getMessage().equals(MESSAGE_FINISH)) {
				...
				String text = String.format("You need to trade me %d slave%s or I won't help you travel to Rome.",
						this.requiredSlaves, (this.requiredSlaves > 1 ? "s" : ""));
				if (GameManager.getOwnSlaveCount() >= this.getRequiredSlaves()) {
					this.setTransitioning(true);
					text = String.format("WELL DONE! Now I can take you to %s.",
							GameManager.getCity(this.getNextLevel()));
					...
					SpeechBubble bubble = SpeechBubble.create(this, text, GameManager.SPEECH_BUBBLE_APPEARANCE,
					...
					bubble.addListener(new SpeechBubbleListener() {
						...
						public void hidden() {
							...
							if (getNextLevel().equals("end")) {
								SpeechBubble bubble2 = SpeechBubble.create(Gatekeeper.this,
										"I see great fortune and plentiful bitches, ummm.... I mean \"riches\" before us...",
										GameManager.SPEECH_BUBBLE_APPEARANCE, GameManager.SPEECH_BUBBLE_FONT);
							...
							Game.loop().perform(1500, () -> {
								// remove player before unloading the environment or the instance's animation
					...
					SpeechBubble.create(this, text, GameManager.SPEECH_BUBBLE_APPEARANCE,
							GameManager.SPEECH_BUBBLE_FONT);
				}
			}
		});
	...
	public void loaded(Environment environment) {
		...
		Game.loop().perform(1000, () -> {
			...
			String goal = MessageFormat.format(ArrayUtilities.getRandom(goalLines), GameManager.getCurrentCity(),
					this.getRequiredSlaves());
			SpeechBubble bubble = SpeechBubble.create(this, rude + " " + goal, GameManager.SPEECH_BUBBLE_APPEARANCE,
					GameManager.SPEECH_BUBBLE_FONT);
	...
	private void performIntroduction() {
		this.tutorial = true;
		SpeechBubble bubble1 = SpeechBubble.create(this, String.format(
				"%s is a terrible dust pit. Let us seek fortune together in Rome, the heart of this glorious empire!",
				Game.world().environment().getMap().getStringValue(MapProperty.MAP_TITLE)),
				GameManager.SPEECH_BUBBLE_APPEARANCE, GameManager.SPEECH_BUBBLE_FONT);
		...
		bubble1.addListener(new SpeechBubbleListener() {
			...
			public void hidden() {
				...
				SpeechBubble bubble2 = SpeechBubble.create(Gatekeeper.this,
						"First, you need to prove your worth to me!", GameManager.SPEECH_BUBBLE_APPEARANCE,
						GameManager.SPEECH_BUBBLE_FONT);
				...
				bubble2.addListener(new SpeechBubbleListener() {
					@Override
					public void hidden() {
						SpeechBubble bubble3 = SpeechBubble.create(Gatekeeper.this,
								"Bring me a slave and I will help you travel to a more glorious place.",
								GameManager.SPEECH_BUBBLE_APPEARANCE, GameManager.SPEECH_BUBBLE_FONT);
						...
						bubble3.addListener(new SpeechBubbleListener() {
							...
							public void hidden() {
								...
								SpeechBubble bubble4 = SpeechBubble.create(Gatekeeper.this,
										"Maybe you could ask that Roman soldier over there nicely to hand over his slave to you.",
										GameManager.SPEECH_BUBBLE_APPEARANCE, GameManager.SPEECH_BUBBLE_FONT);
