...
//      Resources.sounds().get("enemy-engage6.ogg"), Resources.sounds().get("enemy-engage7.ogg"), Resources.sounds().get("enemy-engage8.ogg"), Resources.sounds().get("enemy-engage9.ogg"), Resources.sounds().get("enemy-engage10.ogg"), };
	...
	public Enemy() {
		...
		this.addMessageListener(l -> {
			...
			if (!this.engaged && !this.engaging && l.getMessage().equals(SLAVE_TRIGGER)) {
				SpeechBubble.create(this, ArrayUtilities.getRandom(engagementLines),
		...
		this.addDeathListener(l -> {
			...
			List<Slave> ownedSlaves = Game.world().environment().getByType(Slave.class).stream()
					.filter(sla -> sla.getOwner() != null && sla.getOwner().getMapId() == this.getMapId())
			...
			for (int i = 0; i < spawns; i++) {
				...
				Point2D spawn = new Point2D.Double(this.getCenter().getX() + MathUtilities.randomInRange(-10, 10),
						this.getCenter().getY() + MathUtilities.randomInRange(-10, 10));
				...
				Game.loop().perform(500 * (i + 1), () -> {
					...
					//TODO:Fix sound resource issus 		Game.audio().playSound(Resources.sounds().get("slave-spawn.ogg"));
				});
			}
		});
	}
	...
	public void render(Graphics2D g) {
		...
		if (prep != 0 && !this.isDead()) {
			double angle = GeometricUtilities.calcRotationAngleInDegrees(this.getCollisionBoxCenter(),
	...
	private void initAnimationController() {
		...
		if (this.getSpritePrefix().equals("enemy_silver")) {
			...
			Spritesheet prepareLeft = Resources.spritesheets().load(leftPrepare, "enemy_silver-prepare-left",
					prepare.getSpriteWidth(), prepare.getSpriteHeight());
			...
			final BufferedImage leftPrepare = Imaging.flipSpritesHorizontally(prepare);
			Spritesheet prepareLeft = Resources.spritesheets().load(leftPrepare, "enemy_gold-prepare-left",
					prepare.getSpriteWidth(), prepare.getSpriteHeight());
