...
public class EnemyController extends MovementController<Enemy> {
	...
	public enum EnemyState {
		IDLE, CHASE, STRIKE, PREPARE_CHARGE, CHARGE, PREPARE_STOMP, STOMP,
	}
	...
	public void update() {
		...
		switch (this.state) {
		case STRIKE:
			if (this.getEntity().canCast() && !Player.instance().isDead()
					&& !(GameManager.getGateKeeper() != null && GameManager.getGateKeeper().isTransitioning())) {
				this.getEntity().setAngle(GeometricUtilities.calcRotationAngleInDegrees(this.getEntity().getCenter(),
						Player.instance().getCenter()));
				this.getEntity().getStrike().cast();
			}
			break;
		case CHASE:
			double angle = MathUtilities.randomInRange(-10, 10) + (int) GeometricUtilities
	...
	private void evaluateState() {
		...
		if (!this.preparing
				&& !this.preparingStomp && Player.instance().getCenter().distance(this.getEntity().getCenter()) < this
						.getEntity().getStrike().getAttributes().getImpact().getCurrentValue()
				&& !Player.instance().isDead()) {
			this.state = EnemyState.STRIKE;
			return;
		}

		if (preparingStomp && this.prepareStompStart != 0
				&& Game.time().since(this.prepareStompStart) >= STOMP_PREPARE_DURATION) {
		...
		// STOMP
		if (!this.preparing && this.getEntity().getStomp().getRemainingCooldownInSeconds() == 0
				&& this.getEntity().getType() == EnemyType.gold
				&& Player.instance().getCenter().distance(this.getEntity().getCenter()) < this.getEntity().getStomp()
						.getAttributes().getImpact().getCurrentValue()) {
			...
			if (!this.preparingStomp) {
				if (MathUtilities.probabilityIsTrue(0.5)) {
					...
					if (GeometricUtilities.calcRotationAngleInDegrees(this.getEntity().getCollisionBoxCenter(),
							Player.instance().getCenter()) > 180) {
						prepare += "-left";
					}
		...
		if (!this.preparingStomp && this.getEntity().getCharge().getRemainingCooldownInSeconds() == 0
