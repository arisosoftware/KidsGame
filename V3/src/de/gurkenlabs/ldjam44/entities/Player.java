...
public class Player extends Creature implements IRenderable, IUpdateable {
	...
	private Player() {
		...
		this.addDeathListener(e -> {
			//TODO:Fix sound resource issus 	Game.audio().playSound("fail.ogg");
		});
	...
	private static void spawnHitEmitter(ICombatEntity entity, CombatEntityHitEvent args) {
		...
		Game.loop().perform(130, () -> args.getEntity().getAnimationController()
				.add(new OverlayPixelsImageEffect(120, new Color(255, 0, 0, 200))));
	}
	...
	private void spawnWalkDust(Point2D delta) {
		...
		Point2D walkLocation = new Point2D.Double(
				this.getCollisionBoxCenter().getX() - walkDustSprite.getSpriteWidth() / 2.0,
				this.getCollisionBoxCenter().getY() - walkDustSprite.getSpriteHeight() / 2.0);
	...
	public boolean canTrigger() {
		return Game.world().environment().findCombatEntities(GeometricUtilities.extrude(this.getBoundingBox(), 2))
				.stream().anyMatch(x -> {
	...
	private void initAnimationController() {
		...
		Spritesheet rightJumpSprite = Resources.spritesheets().load(rightJump, "monger-jump-right",
				jump.getSpriteWidth(), jump.getSpriteHeight());
