...
public final class GameManager {
	public enum GameState {
		INGAME, MENU, INGAME_MENU, SLAVES_DEAD
	}
	...
	public static final SpeechBubbleAppearance SPEECH_BUBBLE_APPEARANCE = new SpeechBubbleAppearance(
			new Color(16, 20, 19), new Color(255, 255, 255, 150), new Color(16, 20, 19), 5);
		...
		startups.put("end", () -> {
			...
			Game.loop().perform(1000, () -> {
				...
				Game.loop().perform(2500, () -> {
					SpeechBubble bubble = SpeechBubble.create(waterMonger, ".... ",
					...
					bubble.addListener(new SpeechBubbleListener() {
						...
						public void hidden() {
							...
							SpeechBubble bubble2 = SpeechBubble.create(waterMonger, ":) ",
									GameManager.SPEECH_BUBBLE_APPEARANCE, GameManager.SPEECH_BUBBLE_FONT);
	...
	public static int getOwnSlaveCount() {
		return (int) Game.world().environment().getByType(Slave.class).stream()
				.filter(x -> !x.isDead() && x.getOwner() == null).count();
	}
