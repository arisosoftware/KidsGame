...
/**
 * This static implementation renders an <code>Image</code> to a given
 * <code>Graphics2D</code> object at the specified screen coordinates. This
 * class be very useful when composing a GUI that contains images which are
 * rendered at a certain location on the screen.
 * 
 * @see Image
 * @see Graphics2D
 */
public final class ImageRenderer {
	 ...
	 * @param angle The angle by which the image will be rotated.s
	 */
	public static void renderRotated(final Graphics2D g, final Image image, final double x, final double y,
			final double angle) {
		if (image == null) {
			return;
		}
	...
	public static void renderRotated(final Graphics2D g, final Image image, final Point2D renderLocation,
			final double angle) {
		renderRotated(g, image, renderLocation.getX(), renderLocation.getY(), angle);
	}

	public static void renderScaled(final Graphics2D g, final Image image, final double x, final double y,
			final double scale) {
	...
	public static void renderScaled(final Graphics2D g, final Image image, final Point2D location, final double scaleX,
			final double scaleY) {
	...
	public static void renderTransformed(final Graphics2D g, final Image image, final Point2D renderLocation,
			AffineTransform transform) {
		renderTransformed(g, image, renderLocation.getX(), renderLocation.getY(), transform);
	}

	public static void renderTransformed(final Graphics2D g, final Image image, double x, double y,
			AffineTransform transform) {
		if (transform == null) {
			render(g, image, x, y);
			return;
