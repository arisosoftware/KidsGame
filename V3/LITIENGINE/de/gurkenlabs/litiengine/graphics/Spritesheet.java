...
public final class Spritesheet {
	...
	public BufferedImage getSprite(final int index, final int margin, final int spacing) {
		...
		try {
			final BufferedImage sprite = this.getImage().getSubimage(position.x, position.y, this.spriteWidth,
					this.spriteHeight);
			...
			log.warning("could not read sprite of size [" + this.spriteWidth + "x" + this.spriteHeight
					+ " at position [" + position.x + "," + position.y + "] from sprite'" + this.getName() + "'");
			return null;
		}
	}
	...
	private static void checkDimension(int value, int imageValue, String dimension) {
		if (value <= 0) {
			throw new IllegalArgumentException(
					"Invalid sprite dimensions! Sprite " + dimension + " must to be greater than 0.");
		}

		if (value > imageValue) {
			throw new IllegalArgumentException("Invalid sprite dimensions! Sprite " + dimension + "(" + value
					+ ") cannot be greater than the image " + dimension + "(" + imageValue + ").");
		}
	}
	...
	private Point getLocation(final int index, final int margin, final int spacing) {
		...
		return new Point(margin + column * (this.getSpriteWidth() + spacing),
