...
public abstract class Emitter extends Entity implements IUpdateable, ITimeToLive, IRenderable {
	...
	public Point2D getOrigin() {
		return new Point2D.Double(this.getX() + this.getOriginAlign().getValue(this.getWidth()),
				this.getY() + this.getOriginValign().getValue(this.getHeight()));
	}
	...
	public boolean isFinished() {
		...
		return this.getTimeToLive() > 0 && this.timeToLiveReached()
				|| this.activated && this.lastSpawn > 0 && this.getParticles().isEmpty();
	}
	...
	private void renderParticles(final Graphics2D g, final RenderType renderType) {
		...
		if (Game.screens() != null && Game.world().camera() != null
				&& !Game.world().camera().getViewport().intersects(this.getBoundingBox())) {
			return;
		}
