...
public abstract class Particle implements ITimeToLive {
	 ...
	 * @param ttl    The remaining time to live of the particle.
	...
	public Rectangle2D getBoundingBox(final Point2D origin) {
		return new Rectangle2D.Double(origin.getX() + this.getX(), origin.getY() + this.getY(), this.getWidth(),
				this.getHeight());
	}
	...
	public Point2D getRenderLocation(Point2D effectLocation) {
		...
		// viewport
		Point2D newEffectLocation = Game.screens() != null ? Game.world().camera().getViewportLocation(effectLocation)
				: effectLocation;
	...
	/**
	 * Enabling this check can be very performance hungry and should be used with
	 * caution and only for a small amount of particles.
	 * 
	 * @param ccd If set to true, the collision will be checked continuously by a
	 ...
	 * won't be set.
	 *
	 * @param colorAlpha the new color alpha
	 * 
	...
	public void update(final Point2D emitterOrigin, final float updateRatio) {
		...
		if (this.isFading()) {
			this.opacity = (float) (this.getTimeToLive() > 0
					? (this.getTimeToLive() - this.getAliveTime()) / (double) this.getTimeToLive()
					: 1);
		}
	...
	private boolean checkForCollision(final Point2D emitterOrigin, float targetX, float targetY) {
		if (this.isContinuousCollisionEnabled()) {
			...
			if (this.getCollisionType() != Collision.NONE && Game.physics() != null
		...
		} else if (this.getCollisionType() != Collision.NONE && Game.physics() != null
				&& Game.physics().collides(this.getBoundingBox(emitterOrigin), this.getCollisionType())) {
			if (this.isFadingOnCollision()) {
				this.opacity = 0;
			}
