...
public class FireEmitter extends Emitter {
	 ...
	 * @param life            The number of movements before the new Particle
	 *                        decays.
	 * @param maxAxisMovement the max axis movement
	 */
	public void newParticle(final Color color, final int life, final double maxAxisMovement) {
		...
		this.addParticle(new RectangleFillParticle(size, size, color, life).setX(x).setDeltaX(dx).setDeltaY(dy)
