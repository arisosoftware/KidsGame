...
/**
 * The 2D Render Engine is used to render texts, shapes and entities at their
 * location in the <code>Environment</code> and with respect to the
 * <code>Camera</code> location and zoom.
 * 
 * <p>
 * <i>Internally, it uses the static renderer implementations to actually
 * execute the rendering process. This class basically prepares the specified
 * render subject and passed them to a renderer with the current correct
 * context.</i>
 * </p>
 * 
...
public final class RenderEngine {
	 ...
	 * <code>Game.graphics()</code> instance.</b>
	 * </p>
	 * 
	 * @see Game#graphics()
	 */
	 ...
	 * @param antialias Configure whether or not to render the text with
	 *                  antialiasing.
	 */
	...
	public static void renderImage(Graphics2D g, final Image image, Point2D location) {
		...
		ImageRenderer.render(g, image, viewPortLocation.getX() * Game.world().camera().getRenderScale(),
	...
	public void renderEntities(final Graphics2D g, final Collection<? extends IEntity> entities, final boolean sort) {
		...
		final List<? extends IEntity> entitiesToRender = entities.stream()
				.filter(x -> Game.world().camera().getViewport().intersects(x.getBoundingBox()))
	...
	public void renderEntity(final Graphics2D g, final IEntity entity) {
		...
		if (animationController != null) {
			...
			if (animationController.isAutoScaling()) {
				...
				ImageRenderer.renderScaled(g, img, Game.world().camera().getViewportLocation(entity.getLocation()),
						ratioX, ratioY);
			} else {
				double deltaX = (entity.getWidth() - img.getWidth()) / 2.0;
				double deltaY = (entity.getHeight() - img.getHeight()) / 2.0;

				ImageRenderer.renderTransformed(g, img,
						Game.world().camera().getViewportLocation(entity.getX() + deltaX, entity.getY() + deltaY),
	...
	/**
	 * Sets the global base scale that is used to calculate the actual render scale
	 * of the game.
	 * 
