...
public class GeometricUtilities {
	...
	public static float calcRotationAngleInDegrees(final double centerX, final double centerY, final double targetX,
	...
	public static boolean contains(final Rectangle2D rectangle, final Point2D p) {
		return rectangle.getX() <= p.getX() && rectangle.getY() <= p.getY()
	...
	public static Rectangle2D extrude(Rectangle2D rect, double ext) {
		return new Rectangle2D.Double(rect.getX() - ext, rect.getY() - ext, rect.getWidth() + ext * 2,
	...
	public static List<Line2D.Double> getConstrainingLines(final Area area) {
		...
		for (int i = 0; i < areaPoints.size(); i++) {
			...
			if (nextElement[0] == PathIterator.SEG_LINETO) {
				areaSegments
						.add(new Line2D.Double(currentElement[1], currentElement[2], nextElement[1], nextElement[2]));
	...
	public static Point2D getIntersectionPoint(final Line2D lineA, final Line2D lineB) {
		...
		if (d != 0) {
			...
			if (xi >= Math.min(lineA.getX1(), lineA.getX2()) && xi <= Math.max(lineA.getX1(), lineA.getX2())
					&& yi >= Math.min(lineA.getY1(), lineA.getY2()) && yi <= Math.max(lineA.getY1(), lineA.getY2())) {
	...
	public static List<Point2D> getIntersectionPoints(final Line2D line, final Rectangle2D rectangle) {
		...
		if (p3 != null && !p3.equals(p1) && !p3.equals(p2) && contains(rectangle, p3)
		...
		if (p4 != null && !p4.equals(p1) && !p4.equals(p2) && contains(rectangle, p4)
	...
	public static Line2D[] getLines(final Rectangle2D rectangle) {
		...
		lines[0] = new Line2D.Double(rectangle.getMinX(), rectangle.getMinY(), rectangle.getMinX(),
				rectangle.getMaxY());
		lines[1] = new Line2D.Double(rectangle.getMinX(), rectangle.getMaxY(), rectangle.getMaxX(),
				rectangle.getMaxY());
		lines[2] = new Line2D.Double(rectangle.getMaxX(), rectangle.getMaxY(), rectangle.getMaxX(),
				rectangle.getMinY());
		lines[3] = new Line2D.Double(rectangle.getMaxX(), rectangle.getMinY(), rectangle.getMinX(),
				rectangle.getMinY());
		return lines;
	}
	...
	public static Point2D getPerpendicularIntersection(final Point2D point, final Line2D line) {
		...
		final double k = ((y2 - y1) * (x3 - x1) - (x2 - x1) * (y3 - y1))
	...
	/**
	 * Projects a point from end along the vector (end - start) by the given scalar
	 * amount.
	 *
	...
	public static Point2D[] rayCastPoints(final Point2D point, final Rectangle2D rectangle) {
		...
		for (int i = 0; i < rectPoints.size(); i++) {
			...
			if (intersectionPoints.stream().anyMatch(intersectionPoint -> rectPoints.stream()
					.noneMatch(rectPoint -> equals(rectPoint, intersectionPoint, RAYCAST_EPSILON)))) {
				continue;
			}
	...
	public static Shape scaleRect(final Rectangle2D shape, final int max) {
		...
		final AffineTransform transform = AffineTransform.getScaleInstance(newDimension.getWidth(),
	...
	/**
	 * Shape intersects. WARNING: USE THIS METHOD WITH CAUTION BECAUSE IT IS A VERY
	 * SLOW WAY OF CALCULATING INTERSECTIONS.
	 *
