...
public final class Imaging {
	 ...
	 * <p>
	 * TODO: Add support for different shadow types. Add an ellipse shadow, similar
	 * to the Lepus reatures.
	 * </p>
	 * 
	 ...
	 * Splits up the specified image into a grid with the defined number of rows and
	 * columns.
	 * 
	...
	public static BufferedImage scale(final BufferedImage image, final double factor, boolean keepRatio) {
		...
		return scale(image, (int) Math.max(1, Math.round(width * factor)),
				(int) Math.max(1, Math.round(height * factor)), keepRatio);
	}
	...
	private static BufferedImage flipSprites(final Spritesheet sprite, UnaryOperator<BufferedImage> flipFunction) {
		final BufferedImage flippedSprite = Imaging.getCompatibleImage(sprite.getSpriteWidth() * sprite.getColumns(),
