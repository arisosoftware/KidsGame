...
public class PingLoop extends ClientMessageHandler<PingResponseMessage> implements ILaunchable {
	...
	public PingLoop(final int clientId, final IMessageHandlerProvider provider, final IPacketSender sender,
			final String serverIpAdress, final int port) {
	...
	private class PingThread extends Thread implements ILaunchable {
		...
		public void run() {
			while (!this.isTerminated) {
				...
				final MessagePacket<ClientMessage> packet = new MessagePacket<>(MessageType.PING,
						new ClientMessage(PingLoop.this.clientId));
