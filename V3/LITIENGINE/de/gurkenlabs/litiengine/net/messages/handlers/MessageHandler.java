 ...
 *
 * @param <T> the type of the message that will be validated and handled.
 */
public abstract class MessageHandler<T extends Serializable> implements IMessageHandler {
	...
	public void handle(final byte[] data, final InetAddress address, final int port) {
		...
		if (!this.validate(message)) {
			log.log(Level.INFO, "Data from {0}:{1} is not valid for the messagehandler {2}.",
					new Object[] { address.getHostAddress(), port, this.getClass().getSimpleName() });
			return;
		}
	 ...
	 * validation mechanism for the messages sent. E.g. one could test if a certain
	 * field of the message is not null.
	 *
	 * @param message The message object that is validated.
