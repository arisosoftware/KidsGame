...
public class LightSource extends Entity implements IRenderable {
	 ...
	 */
	private static final Color[] SHADOW_GRADIENT_COLORS = new Color[] { new Color(0, 0, 0, .3f),
	...
	private static Area getObstructedVisionArea(final IEntity mob, final Point2D center) {
		...
		final Point2D relativeCenter = Game.world().camera()
		...
		final Point2D shadowRenderLocation = Game.world().camera()
				.getViewportLocation(new Point2D.Double(shadowEllipse.getX(), shadowEllipse.getY()));
		final Ellipse2D relativeEllipse = new Ellipse2D.Double(shadowRenderLocation.getX(), shadowRenderLocation.getY(),
				shadowEllipse.getWidth(), shadowEllipse.getHeight());
	...
	private void renderShadows(final Graphics2D g) {
		...
		final Paint gradientPaint = new RadialGradientPaint(Game.world().camera().getViewportDimensionCenter(this),
				SHADOW_GRADIENT_SIZE, SHADOW_GRADIENT_FRACTIONS, SHADOW_GRADIENT_COLORS);
		...
		for (final ICombatEntity mob : Game.world().environment().getCombatEntities()) {
			...
			final Shape obstructedVision = getObstructedVisionArea(mob,
