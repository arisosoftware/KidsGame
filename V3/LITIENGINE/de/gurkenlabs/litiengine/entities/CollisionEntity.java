...
public abstract class CollisionEntity extends Entity implements ICollisionEntity {
	...
	public static Rectangle2D getCollisionBox(final Point2D location, final double entityWidth,
			final double entityHeight, final double collisionBoxWidth, final double collisionBoxHeight,
			final Align align, final Valign valign) {
	...
	public Rectangle2D getCollisionBox(final Point2D location) {
		final double newCollisionBoxWidth = this.getCollisionBoxWidth() != -1 ? this.getCollisionBoxWidth()
				: this.getWidth() * WIDTH_FACTOR;
		final double newCollisionBoxHeight = this.getCollisionBoxHeight() != -1 ? this.getCollisionBoxHeight()
				: this.getHeight() * HEIGHT_FACTOR;

		return getCollisionBox(location, this.getWidth(), this.getHeight(), newCollisionBoxWidth, newCollisionBoxHeight,
				this.getCollisionBoxAlign(), this.getCollisionBoxValign());
	}
	...
	public void setCollisionType(Collision type) {
		...
		if (this.getEnvironment() != null && this.getEnvironment().isLoaded()) {
			// re-add the entity to the physics engine so it will be treated with the
			// updated collision type
