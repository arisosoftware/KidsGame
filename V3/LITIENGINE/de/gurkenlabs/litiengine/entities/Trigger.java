...
public class Trigger extends CollisionEntity implements IUpdateable {
	...
	public String sendMessage(final Object sender, final String message) {
		if (this.activationType == TriggerActivation.COLLISION && sender != null && sender instanceof IEntity
		...
		if (sender instanceof IEntity) {
			...
			if (this.activators.isEmpty() || this.activators.contains(ent.getMapId())) {
				...
				log.log(Level.INFO,
						...
						new Object[] { this.getName(), ent.getMapId() });
				return Boolean.toString(false);
			}
		}
	...
	private boolean activate(final IEntity activator, final int tar) {
		if (this.isOneTimeTrigger && this.isActivated || this.getActivationType() == TriggerActivation.COLLISION
				&& activator != null && this.activated.contains(activator)) {
			return false;
		}
		...
		for (final int target : triggerTargets) {
			...
			if (entity == null) {
				log.log(Level.WARNING,
						...
						new Object[] { this.getName(), target });
				continue;
			}
