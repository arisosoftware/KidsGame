...
public abstract class Ability implements IRenderable {
	...
	public boolean canCast() {
		return !this.getExecutor().isDead()
				&& (this.getCurrentExecution() == null || this.getCurrentExecution().getExecutionTicks() == 0
						|| Game.time().since(this.getCurrentExecution().getExecutionTicks()) >= this.getAttributes()
								.getCooldown().getCurrentValue());
	}
	...
	public Point2D getOrigin() {
		...
		case COLLISIONBOX_CENTER:
			return new Point2D.Double(this.executor.getCollisionBox().getCenterX(),
					this.executor.getCollisionBox().getCenterY());
		case DIMENSION_CENTER:
			return this.executor.getCenter();
		case CUSTOM:
			if (this.origin != null) {
				return new Point2D.Double(this.executor.getX() + this.origin.getX(),
						this.executor.getY() + this.origin.getY());
			}
			break;
		case LOCATION:
		default:
	...
	public float getRemainingCooldownInSeconds() {
		...
		return (float) (!this.canCast()
				? (this.getAttributes().getCooldown().getCurrentValue()
						- Game.time().since(this.getCurrentExecution().getExecutionTicks())) * 0.001
				: 0);
	}
	 ...
	 * ability.
	 * 
	 * @param origin The origin that defines the execution offset for this
	 *               {@link Ability}.
	 */
	...
	protected Shape internalCalculateImpactArea(final double angle) {
		...
		// project
		final Point2D appliedRange = GeometricUtilities.project(new Point2D.Double(arcX, arcY), angle,
				this.getAttributes().getRange().getCurrentValue() * 0.5);
		...
		return new Arc2D.Double(appliedRange.getX(), appliedRange.getY(), impact, impact, start, impactAngle,
				Arc2D.PIE);
	}
