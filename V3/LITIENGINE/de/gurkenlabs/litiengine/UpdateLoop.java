...
public class UpdateLoop extends Thread implements AutoCloseable, ILoop {
	...
	/**
	 * The loop implementation, executing the <code>process()</code> method which
	 * does the actual work. It also tracks the processing time and the total number
	 * of performed ticks while making sure that the expected tick rate is met by
	...
	/**
	 * Performs the actual workload of a tick. This base implementation just calls
	 * the update method on all registered instances. For derived loop
	 * implementations this is more sophisticated.
	 */
	...
	/**
	 * This method determines how long the current tick should be delayed to match
	 * the expected delta time for the specified tick rate. It then delays the
	 * execution of this loop by pausing the thread for the necessary delay.
	 * 
	...
	protected double delay() {
		...
		// since thread sleep only supports long values and no double values, there will
		// always be some error that we need to account for
		// we'll aggregate the error until its absolute value is greater than 1 and then
		// add it to the current delay and subtract it from the current delay error
