...
public class MovementController<T extends IMobileEntity> implements IMovementController {
	...
	public void handleMovement() {
		...
		final double dec = this.getEntity().getDeceleration() == 0 ? maxPixelsPerTick
				: decelerationRatio * maxPixelsPerTick;
	...
	public Force getForce(String identifier) {
		...
		return this.getActiveForces().stream()
				.filter(x -> x.getIdentifier() != null && x.getIdentifier().equals(identifier)).findFirst()
				.orElse(null);
	}
	...
	protected void moveEntity(double deltaX, double deltaY) {
		final Point2D newLocation = new Point2D.Double(this.getEntity().getX() + deltaX,
				this.getEntity().getY() + deltaY);
		final Point2D oldLocation = this.getEntity().getLocation();
		Game.physics().move(this.getEntity(), newLocation);

		final Point2D delta = new Point2D.Double(this.getEntity().getX() - oldLocation.getX(),
				this.getEntity().getY() - oldLocation.getY());
	...
	private void handleForces() {
		...
		try {
			...
			for (final Force force : this.activeForces) {
				...
				final double angle = GeometricUtilities.calcRotationAngleInDegrees(collisionBoxCenter,
						force.getLocation());
				final double strength = Game.loop().getDeltaTime() * 0.001f * force.getStrength()
			...
			final Point2D target = new Point2D.Double(this.getEntity().getX() + deltaX,
					this.getEntity().getY() + deltaY);
