...
public class Force {
	 ...
	 * @param size     The size of this force (used to determine if/when an entity
	 *                 has reached the force)
	 */
	...
	public boolean hasReached(final ICollisionEntity entity) {
		return new Ellipse2D.Double(this.getLocation().getX() - this.size * 0.5,
				this.getLocation().getY() - this.size * 0.5, this.size, this.size).intersects(entity.getCollisionBox());
	}
