...
public class LightSourceMapObjectLoader extends MapObjectLoader {
	...
	public Collection<IEntity> load(Environment environment, IMapObject mapObject) {
		if (MapObjectType.get(mapObject.getType()) != MapObjectType.LIGHTSOURCE) {
			throw new IllegalArgumentException("Cannot load a mapobject of the type " + mapObject.getType()
					+ " with a loader of the type " + LightSourceMapObjectLoader.class);
		}
		...
		final LightSource.Type lightType = mapObject.getEnumValue(MapObjectProperty.LIGHT_SHAPE,
				LightSource.Type.class);
	...
	protected LightSource createLightSource(IMapObject mapObject, int intensity, Color color,
			LightSource.Type lightType, boolean active) {
