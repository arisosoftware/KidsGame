...
public class CreatureMapObjectLoader extends MapObjectLoader {
	...
	/**
	 * <p>
	 * Registers a custom {@link Creature} implementation that can be automatically
	 * provided by this {@link MapObjectLoader}.
	 * </p>
	 * 
	 * <p>
	 * <b>This should only be used if the particular implementation doesn't require
	 * any additional map object properties to be initialized.</b>
	 * </p>
	 * 
	 ...
	 * 
	 * <p>
	 * The latter is particularly useful for classes that can have different sprite
	 * sheets, i.e. share the same logic but might have a different appearance.
	 * </p>
	 * 
	...
	public Collection<IEntity> load(Environment environment, IMapObject mapObject) {
		if (MapObjectType.get(mapObject.getType()) != MapObjectType.CREATURE) {
			throw new IllegalArgumentException("Cannot load a mapobject of the type " + mapObject.getType()
					+ " with a loader of the type " + CreatureMapObjectLoader.class);
		}
		...
		Creature creature = this.createNewCreature(mapObject, spriteSheet,
				mapObject.getStringValue(MapObjectProperty.SPAWN_TYPE));
		...
		creature.setFacingDirection(
				mapObject.getEnumValue(MapObjectProperty.SPAWN_DIRECTION, Direction.class, Direction.RIGHT));
	...
	protected Creature createNewCreature(IMapObject mapObject, String spriteSheet, String spawnType) {
		// for each known custom creature type, check if it was registered for the
		// specified spriteSheetName
		// if so: create an instance of the custom class instead of the default Creature
		// class
		...
		// if no custom creature type war registered for the spriteSheet, we just create
