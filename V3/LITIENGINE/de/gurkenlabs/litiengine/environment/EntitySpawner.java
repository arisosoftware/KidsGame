...
public abstract class EntitySpawner<T extends IEntity> implements IEntitySpawner<T> {
	 ...
	 * @param loop        the loop
	 * @param spawnpoints the spawnpoints
	 * @param interval    the interval
	 * @param amount      the amount
	 */
	public EntitySpawner(final Environment environment, final IGameLoop loop, final List<Spawnpoint> spawnpoints,
			final int interval, final int amount) {
	...
	/**
	 * Spawn new entities, depending on the <code>SpawnMode</code>, spawnAmount,
	 * spawnDelay, and spawnInterval of an <code>EntitySpawner</code>.
	 * 
	 * @see SpawnMode
	 */
	...
	private class SpawnThread extends Thread {
		 ...
		 *
		 * @param point  the <code>Spawnpoint</code> where entities will be spawned.
