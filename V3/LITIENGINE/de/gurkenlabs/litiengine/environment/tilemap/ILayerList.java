...
public interface ILayerList extends ICustomPropertyProvider {
	...
	/**
	 * Gets the <code>IMapObjectLayer</code> containing a given
	 * <code>IMapObject</code>.
	 *
	...
	public default IMapObjectLayer getMapObjectLayer(IMapObject mapObject) {
		for (IMapObjectLayer layer : this.getMapObjectLayers()) {
			Optional<IMapObject> found = layer.getMapObjects().stream().filter(x -> x.getId() == mapObject.getId())
					.findFirst();
			if (found.isPresent()) {
				return layer;
			}
	...
	public default IMapObjectLayer getMapObjectLayer(String layerName) {
		Optional<IMapObjectLayer> layer = this.getMapObjectLayers().stream().filter(x -> x.getName().equals(layerName))
				.findFirst();
		return layer.orElse(null);
	}

	public default IMapObjectLayer getMapObjectLayer(int layerId) {
		Optional<IMapObjectLayer> layer = this.getMapObjectLayers().stream().filter(x -> x.getId() == layerId)
				.findFirst();
		return layer.orElse(null);
	}
	...
	/**
	 * Gets all map objects in the layer list that belong to the types passed as a
	 * parameter.
	 *
	...
	/**
	 * Gets all map objects in the layer list using the map IDs passed as a
	 * parameter. Please note that map IDs are intended to be unique identifiers for
	 * <code>IMapObject</code>s (and their corresponding <code>Entity</code>). This
	 * method is just a way of checking for non-unique IDs and re-assigning them
	 ...
	 *
	 * @param mapId the map id of the desired <code>IMapObject</code>
	 ...
	 *
	 * @param mapId the map id of the <code>IMapObject</code> we want to remove
	 */
