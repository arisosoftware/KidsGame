...
public class MapOrientations {
	/**
	 * <p>
	 * An {@code IMapOrientation} for orthogonal maps, consistent with the behavior
	 * of Tiled.
	 *
	 * <p>
	 * With this orientation, tiles are treated as rectangles with dimensions equal
	 * to the tile size, layed out in rows and columns starting with the origin.
	 */
	 ...
	 *
	 * <p>
	 * With this orientation, the shapes of tiles are transformed into an isometric
	 * coordinate system via the transformation
	 * {@code (x, y) -> ((x-y)/2, (x+y)/2)}. Rectangles within this coordinate
	 * system are rendered as diamond shapes. Points are also translated such that
	 * every tile in a map fits in the first quadrant.
	 *
	 * <p>
	 * This orientation does <em>not</em> transform map objects into its coordinate
	 * system. Maps with an odd-numbered tile width or height will throw
	...
	/**
	 * <p>
	 * A staggered isometric {@code IMapOrientation}, consistent with the behavior
	 * of Tiled.
	 *
	 * <p>
	 * This orientation is similar to the isometric orientation, but the tile
	 * coordinates are changed to be arranged in a rectangular area. The locations
	 * of tiles are positioned using rectangular coordinates, with tiles with parity
	 * on the stagger axis matching the stagger index filling the gaps between the
	 * other tiles.
	 *
	 * <p>
	 * This orientation requires that a stagger axis and stagger index be set. If
	 * either are missing, an {@code IllegalArgumentException} will be thrown. Like
	 * the standard isometric orientation, this orientation will also throw an
	 * {@code IllegalArgumentException} when used with a map with an odd-numbered
	 ...
	 *
	 * <p>
	 * Tiles are arranged in the same manner as the staggered isometric orientation
	 * (the staggered isometric orientation can be viewed as a special case of this
	 * orientation with a hex side length of 0), with extra space between tiles (the
	 * hex side length) added on the stagger axis to allow for a hexagonal shape.
	 *
	 * <p>
	 * This orientation has the same requirements as staggered isometric, with the
	 * additional restriction that the hex side length must be an even number.
	 */
	public static final IMapOrientation HEXAGONAL = new Hexagonal();

	/**
	 * Determines the appropriate {@code IMapOrientation} instance for the given
	 * name. If no such orientation exists, this method returns {@code null}.
	 *
	...
	private static class Isometric extends Base {
		...
		public Point getLocation(int x, int y, IMap map) {
			checkSizeParity(map);
			return new Point((x - y + map.getHeight() - 1) * map.getTileWidth() / 2,
		...
		public Point getTile(double x, double y, IMap map) {
			checkTileSize(map);
			checkSizeParity(map);
			// transform our coordinates to put the top-left corner at the origin and make
			// tiles 1x1
			x /= map.getTileWidth();
			y /= map.getTileHeight();
			x -= 0.5 * map.getHeight();
			// this uses ceil(y)-1 instead of floor(y) to be consistent with getShape; see
			// the definition of "insideness" in the Shape class
