...
public final class MapUtilities {
	...
	public static Rectangle2D getTileBoundingBox(final IMap map, final Rectangle2D box) {
		...
		int minTileX = map.getOrientation().getName().equals(MapOrientations.ISOMETRIC.getName()) ? minTilePoint.x
				: MathUtilities.clamp(minTilePoint.x, 0, map.getWidth() - 1);
		int minTileY = map.getOrientation().getName().equals(MapOrientations.ISOMETRIC.getName()) ? minTilePoint.y
				: MathUtilities.clamp(minTilePoint.y, 0, map.getHeight() - 1);
		int maxTileX = map.getOrientation().getName().equals(MapOrientations.ISOMETRIC.getName()) ? maxTilePoint.x
				: MathUtilities.clamp(maxTilePoint.x, 0, map.getWidth() - 1);
		int maxTileY = map.getOrientation().getName().equals(MapOrientations.ISOMETRIC.getName()) ? maxTilePoint.y
				: MathUtilities.clamp(maxTilePoint.y, 0, map.getWidth() - 1);
		final Rectangle2D minTileBounds = map.getOrientation()
				.getBounds(new Point(MathUtilities.clamp(minTileX, 0, map.getWidth() - 1),
						MathUtilities.clamp(minTileY, 0, map.getHeight() - 1)), map);
		final Rectangle2D maxTileBounds = map.getOrientation()
				.getBounds(new Point(MathUtilities.clamp(maxTileX, 0, map.getWidth() - 1),
						MathUtilities.clamp(maxTileY, 0, map.getHeight() - 1)), map);

		return new Rectangle2D.Double(minTileBounds.getX(), minTileBounds.getY(),
	 ...
	 * 
	 * @param staggerIndex the staggerIndex property of the map. Every second row
	 *                     (or column, depending on the {@link StaggerAxis} of the
	 *                     map is staggered half a tile.
	 * @param index        the index of the current row or column for which we want
	 *                     to determine if it's staggered or not.
	 * @return a boolean representing if the row or column with the given index is
	 *         staggered.
	 */
	...
	/**
	 * Searches for the tile set that contains the specified tile, identified by the
	 * grid id.
	 *
	 * @param map  the map
	 * @param tile the tile
