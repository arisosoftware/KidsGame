...
public class AStarPathFinder extends PathFinder {
	...
	private Path findAStarPath(AStarNode startNode, AStarNode targetNode) {
		...
		while (!opened.isEmpty()) {
			// after the first iteration, this will also contained the newly found neighbors
			// that were added in the last iteration
	...
	/**
	 * Updates the costs and the predecessor of all neighbors of the specified
	 * <code>currentNode</code>.<br>
	 * If a neighbor was previously not part of the <code>opened</code> list it will
	 * be added to it.<br>
	 * If a neighbor is already closed, it will be ignored.<br>
	 * If the {@link AStarNode#isWalkable()} method of a neighbor returns
	 * <code>false</code> it will also not be considered.
	 * 
	 ...
	 * @param opened      The list of all the opened nodes of the path-finding
	 *                    operation.
	 * @param closed      The list of all the closed nodes of the path-finding
	 *                    operation.
	 */
	private void updateAndOpenNeighborNodes(AStarNode currentNode, AStarNode targetNode, List<AStarNode> opened,
	...
	private static AStarNode findNodeWithLowestCost(List<AStarNode> openedNodes) {
		...
		// F-cost (aka. total costs) are considered first. If they are equal, the H-cost
		// is checked subsequently
	...
	/**
	 * Retraces the found path from the targetNode back to the startNode by making
	 * use of the {@link AStarNode#getPredecessor()}.
	 * <ol>
	 * <li>Adds all predecessors to a list of nodes that will be visited by the
	 * path.</li>
	 * <li>Invert the list.</li>
	 * <li>Create a new {@link Path2D} by iterating all nodes in the list.</li>
	 * <li>Wrap the {@link Path2D} object into a {@link Path} to provide information
	 * about the start, target and points of the path.</li>
	 * </ol>
	 * 
