...
public class AStarGrid implements IRenderable {
	...
	public void render(Graphics2D g) {
		...
		for (int x = startX; x <= endX; x++) {
			for (int y = startY; y <= endY; y++) {
				...
				if (node.isWalkable()) {
					RenderEngine.renderShape(g,
							new Rectangle2D.Double(node.getLocation().x - 0.25, node.getLocation().y - 0.25, 0.5, 0.5));
				} else {
	...
	protected void assignPenalty(AStarNode node) {
		...
		for (Prop prop : Game.world().environment().getProps()) {
			if (!prop.hasCollision() || !prop.isIndestructible()
	...
	private void addDiagonalNode(final List<AStarNode> neighbors, AStarNode node, AStarNode diagonalNeighbor1,
			AStarNode diagonalNeighbor2) {
		// only add diagonal neighbors when they are not on a corner
		if (node != null && this.isDiagonalCornerMovementAllowed() || node != null && diagonalNeighbor1 != null
				&& diagonalNeighbor1.isWalkable() && diagonalNeighbor2 != null && diagonalNeighbor2.isWalkable()) {
			neighbors.add(node);
		}
	}
	...
	private void populateGrid(final int gridSizeX, final int gridSizeY) {
		for (int x = 0; x < gridSizeX; x++) {
			for (int y = 0; y < gridSizeY; y++) {
				final Rectangle nodeBounds = new Rectangle(x * this.nodeSize, y * this.nodeSize, this.nodeSize,
						this.nodeSize);
				final AStarNode node = new AStarNode(!Game.physics().collides(nodeBounds, Collision.STATIC), nodeBounds,
						x, y);
