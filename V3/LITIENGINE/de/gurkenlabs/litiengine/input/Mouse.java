...
public class Mouse implements IMouse {
	 ...
	 * 
	 * @throws AWTException In case the {@link Robot} class could not be
	 *                      initialized.
	 */
	protected Mouse() throws AWTException {
		...
		this.location = new Point2D.Double(Game.world().camera().getViewport().getCenterX(),
	...
	public Point2D getMapLocation() {
		return Game.world().camera()
				.getMapLocation(new Point2D.Double(this.getLocation().getX() / Game.world().camera().getRenderScale(),
	...
	public void setLocation(final Point2D adjustMouse) {
		...
		final MouseEvent mouseEvent = new MouseEvent(Game.window().getRenderComponent(), MouseEvent.MOUSE_MOVED, 0, 0,
				(int) this.getLocation().getX(), (int) this.getLocation().getY(), 0, false, MouseEvent.NOBUTTON);
	...
	private MouseEvent createEvent(final MouseEvent original) {
		return new MouseEvent(original.getComponent(), original.getID(), original.getWhen(), original.getModifiers(),
				(int) this.getLocation().getX(), (int) this.getLocation().getY(), original.getXOnScreen(),
				original.getYOnScreen(), original.getClickCount(), original.isPopupTrigger(), original.getButton());
	}
	 ...
	 *
	 * @param mouseLocation The location of the original mouse.
	 */
	private void setLocation(final MouseEvent e) {
		...
		if (!this.grabMouse) {
			...
			this.lastLocation = new Point(e.getPoint().x - Game.window().getRenderComponent().getCursorOffsetX(),
					e.getPoint().y - Game.window().getRenderComponent().getCursorOffsetY());
		} else {
