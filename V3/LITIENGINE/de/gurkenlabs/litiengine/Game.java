...
/***
 * <p>
 * The <code>Game</code> class is without any doubt one of the classes that you
 * will call a lot when creating a game with the LITIengine. It is designed to
 * be the static container that provides access to all important aspects of the
 * engine, e.g. it holds the <code>GameInfo</code>, the
 * <code>RenderEngine</code>, the <code>SoundEngine</code> and many other major
 * components.
 * </p>
 * <p>
 * We designed the API such that all important parts that make up the game are
 * directly accessible via the <code>Game</code> class in a static manner. To be
 * a little bit more technical, it is essentially a collection of core Singleton
 * instances.
 * </p>
 * <p>
 * This class will also be your starting point when setting up a new LITIengine
 * project. In order to launch your game, you need to at least call
 * {@link Game#init(String...)} and {@link Game#start()} from your programs
 * <code>main(String[])</code> method.
 * </p>
 * <p>
 * Additionally, it provides an interface to hook up event listeners (e.g.
 * <code>GameListener</code> or <code>EnvironmentLoadedListener</code>) for the
 * most basic operations of a Game life cycle. 
 * </p>
 * 
 * @see GameListener
...
public final class Game {
	 ...
	 * disabled by calling this method or providing the command line argument
	 * {@link #COMMADLINE_ARG_RELEASE} when running the game.
	 * 
	 ...
	 * <code>GameWindow</code> or not. This can only be set before the game has been
	 * initialized with the <code>Game.init(String...)</code> method. Afterwards it
	 * doesn't have an effect anymore. If enabled, the
	 * <code>ScreenManager#setVisible(boolean)</code> method won't be set to true
	 * and the <code>RenderLoop</code> won't be started. Also the
	 * <code>Camera</code> won't be updated.
	 * 
	...
	/**
	 * This flag globally controls the game's debugging state. If enabled, debugging
	 * functionality (e.g. rendering collision boxes) can potentially be enabled in
	 * the configuration.
	 * 
	 ...
	 * This can be used to define meta information about your game, like it's name,
	 * version or web site.<br>
	 * <br>
	 * <i>It's also possible to provide additional custom information using the
	 * method group <br>
	 ...
	 * It contains default engine settings for the game client, graphics, audio,
	 * input and debugging.<br>
	 * Additionally, it can be used to register and manage custom settings that are
	 * specific to your game.
	 * <p>
	 * <i> Elements of this configuration are also presented in a config.properties
	 * file in the game's root directory. <br>
	 * This way its possible to adjust elements without having to recompile the
	 * game. </i>
	 * </p>
	 * 
	...
	/**
	 * Gets basic client metrics about the game's runtime. This includes information
	 * about network, the frames-per-second or the updates-per-second and the used
	 * memory.
	 * 
	 * <p>
	 * <i> This information can be rendered by setting <br>
	 * <code>Game.config().client().setShowGameMetrics(boolean)</code> to true or
	 * <br>
	 * <code>cl_showGameMetrics=true</code> in the config.settings. </i>
	 * </p>
	 * 
	 ...
	 * This class e.g. provides the possibility to set a title, provide an icon or
	...
	/**
	 * Gets the engine's <code>SoundEngine</code> component that can be used to play
	 * sounds and music.<br>
	 * Sound can be loaded and accessed using the <code>Resources</code> API and are
	 * managed by the<br>
	 * <code>Resources.sounds()</code> resource container.
	 * 
	 * <p>
	 * <i> Upon playing a sound, the engine returns an <code>SoundPlayback</code>
	 * instance that can then be used to further control the audio line. </i>
	 * </p>
	 * 
	...
	/**
	 * Gets the engine's <code>PhysicsEngine</code> component that can be used to
	 * detect and resolve collision and move entities with respect to all collision
	 * entities on the environment.<br>
	 * The boundaries of the loaded environment also pose a "non-walkable" area that
	 * will be taken into account when moving entities with this engine.
	 * 
	 * <p>
	 * <i>It is also possible to manually register static collision
	 * <code>Rectangles</code> that can further restrict the game world.</i>
	 * </p>
	 * 
	...
	/**
	 * Gets the engine's <code>RenderEngine</code> component that is used to render
	 * <code>Images, Shapes or Text</code> with respect to the environment and the
	 * render scale and the <code>Camera</code>.
	 * 
	 * <p>
	 * <i>In case you want to render something in a static manner that is unrelated
	 * to the environment, you can use the engine's different static
	...
	/**
	 * Gets the game's main loop that is used to execute and manage all game logic
	 * apart from rendering and input processing.<br>
	 * You can attach any <code>Updatable</code> instance to this loop if you want
	 * to execute custom game logic that is executed at the configured updaterate.
	 * 
	 * <p>
	 * <i>The LITIengine has separate loops for game logic, rendering and input
	 * processing. <br>
	 * This prevents them from interfering with each other and also properly
	 * separates tasks by their category.</i>
	 * </p>
	 * 
	 * 
	 ...
	 * 
	 * <p>
	 * <i>We need an own update loop because otherwise input won't work if the game
	 * has been paused.</i>
	 * </p>
	 * 
	 ...
	 * <code>RenderComponent</code>.<br>
	 * This internally renders the currently active screen which passes the
	 * <code>Graphics2D</code> object to all <code>GuiComponents</code> and the
	 * Environment for rendering. This loop will try to execute at the configured
	 * frames-per-second and limit the frames to this value.
	 * 
	 * <p>
	 * <i>It's also possible to register <code>Updatable</code> instances to this
	 * loop which is useful if you want to execute something that is directly
	 * related to the rendering process and needs to be executed right before the
	 * game's rendering starts.</i>
	 * </p>
	 * 
	...
	/**
	 * Gets the game's <code>ScreenManager</code> that is responsible for organizing
	 * all <code>Screens</code> of your game and providing the currently active
	 * <code>Screen</code> that is used to render the current
	 * <code>Environment</code>.<br>
	 * Screens are the containers that allow you to organize the visible contents of
	 * your game and are identified and addressed by a unique name.
	 * 
	 * <p>
	 ...
	 * <code>Environments</code> and provides the currently active
	 * <code>Environment</code> and <code>Camera</code>.<br>
	 * <p>
	 * The <code>GameWorld</code> returns the same instance for a particular
	 * map/mapName until the <code>GameWorld.reset(String)</code> method is called.
	 * </p>
	 * 
	 * Moreover, it provides the possibility to attach game logic via
	 * <code>EnvironmentListeners</code> to different events of the
	 ...
	 * <li>set the programs <code>Locale</code> according to the configured
	 * values.</li>
	 * <li>initialize and attach core components like the
	 * <code>PhysicsEngine</code></li>
	 ...
	 * </ul>
	 * 
	 * @param args The arguments passed to the programs entry point.
	 */
	public static synchronized void init(String... args) {
		...
		if (!isInNoGUIMode()) {
			...
			if (config().debug().isDebugEnabled()) {
				graphics().onEntityRendered(
						e -> DebugRenderer.renderEntityDebugInfo(e.getGraphics(), e.getRenderedObject()));
			}

			window().getRenderComponent().onFpsChanged(fps -> metrics().setFramesPerSecond(fps));
			window().setIconImages(Arrays.asList(Resources.images().get("liti-logo-x16.png"),
					Resources.images().get("liti-logo-x20.png"), Resources.images().get("liti-logo-x32.png"),
	...
	/***
	 * <p>
	 * Starts the <code>GameLoops</code> and other components. After this method is
	 * called, the engine will start to render contents of the current
	 * <code>Screen</code> of the <code>ScreenManager</code>, the
	 * <code>SoundEngine</code> will start to playback <code>Sounds</code> and the
	 * different input devices (e.g. <code>Mouse</code>, <code>Keyboard</code>) will
	 * start to process player input.
	 * </p>
	 * <p>
	 * When the <code>Game</code> has started up successfully, it'll callback to the
	 * registered <code>GameListeners</code>.
	 * </p>
	 * 
	...
	/**
	 * Sets the <code>Game's</code> basic information by the specified
	 * <code>GameInfo</code> instance.
	 * <p>
	 * <i>Typically, this should not be called manually because the
	 * <code>Game</code> already provides a <code>GameInfo</code> object which can
	 * be adjusted.<br>
	 * If you just want to edit some of it's information, use the provided instance
	 * of {@link Game#info()}. </i>
	 * </p>
	 * 
	 * @param info The <code>GameInfo</code> that contains the basic information for
	 *             the game.
	 * 
	 * @see Game#info()
	 * @see GameInfo
	...
	/**
	 * Sets the <code>Game's</code> basic information by loading the
	 * <code>GameInfo</code> from the specified path to an XML file.
	 * 
	 * @param gameInfoFile The path to the XML file that contains the serialized
	 *                     <code>GameInfo</code>.
	 * 
