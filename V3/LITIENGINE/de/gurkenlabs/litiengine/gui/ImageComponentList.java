...
public class ImageComponentList extends GuiComponent {
	...
	public ImageComponentList(final double x, final double y, final double width, final double height, final int rows,
			final int columns, final List<Image> images, final Spritesheet background) {
		...
		if (!initialized) {
			...
			for (int j = 0; j < this.getRows(); j++) {
				for (int i = 0; i < this.getColumns(); i++) {
					...
					final ImageComponent cell = this.createNewEntry(this.getX() + i * (this.columnWidth + this.xOffset),
							this.getY() + j * (this.rowHeight + this.yOffset), this.columnWidth, this.rowHeight,
							this.getBackground(), "", img);
					this.cells.add(cell);
				}
			}
	...
	protected ImageComponent createNewEntry(final double x, final double y, final double width, final double height,
			final Spritesheet spritesheet, final String text, final Image image) {
