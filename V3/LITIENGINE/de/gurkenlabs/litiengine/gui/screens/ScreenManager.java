...
/**
 * The <code>ScreenManager</code> holds instances of all available screen and
 * handles whenever a different <code>Screen</code> should be shown to the
 * player. It provides the currently active Screen for the Game’s
 * <code>RenderComponent</code> which calls the
 * <code>Screen.render(Graphics2D)</code> method on every tick of the
 * <code>RenderLoop</code>. Overwriting this method provides the ability to
 * define a customized render pipeline that suits the need of a particular
 * Screen implementation. With the GameScreen, the LITIengine provides a simple
 * default Screen implementation that renders the current
 * <code>Environment</code> and all its <code>GuiComponents</code>.
 * 
 * @see Screen
 * @see RenderComponent
 * @see GameScreen
...
public final class ScreenManager {
	...
	public void display(final String screenName) {
		...
		if (this.current() != null && this.current().getName().equalsIgnoreCase(screenName)
				|| this.screens.stream().noneMatch(element -> element.getName().equalsIgnoreCase(screenName))) {
	...
	public Screen get(String screenName) {
		Optional<Screen> opt = this.screens.stream().filter(element -> element.getName().equalsIgnoreCase(screenName))
				.findFirst();
