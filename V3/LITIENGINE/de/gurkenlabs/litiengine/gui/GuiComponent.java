...
/**
 * The abstract Class GuiComponent provides all properties and methods needed
 * for screens, built-in, and custom GUI components such as buttons, sliders,
 * etc... It includes mouse event handling, different hovering states and
...
public abstract class GuiComponent implements MouseListener, MouseMotionListener, MouseWheelListener, IRenderable {
	...
	/**
	 * Instantiates a new gui component with the dimension (0,0) at the given
	 * location.
	 *
	 * @param x the x
	 * @param y the y
	...
	/**
	 * Instantiates a new gui component at the point (x,y) with the dimension
	 * (width,height).
	 *
	 * @param x      the x
	 * @param y      the y
	...
	/**
	 * Gets the margin size between the GuiComponent's left and right border and the
	 * Text bounds.
	 *
	...
	/**
	 * Gets the entire Text associated with this GuiComponent. Parts of the Text may
	 * get cropped and can therefore be invisible. To retrieve only the visible part
	 * of the text, use <code>GuiComponent.getTextToRender(Graphics2D g)</code>.
	 *
	...
	/**
	 * Gets only the non-cropped bits of Text visible on this GuiComponent.m To
	 * retrieve only the entire text associated with this GuiComponent, use
	 * <code>GuiComponent.getText()</code>.
	 *
	...
	/**
	 * Checks if the cursor bounding box intersects with this GuiCOmponent's
	 * bounding box.
	 *
	...
	/**
	 * Add a callback that is being executed if this GuiComponent is hovered with
	 * the mouse.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the mouse is pressed and moving
	 * around while within the bounds of this GuiComponent.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the mouse enters the bounds of this
	 * GuiComponent.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the mouse leaves the bounds of this
	 * GuiComponent.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the mouse is moving around while
	...
	/**
	 * Add a callback that is being executed if the mouse is continually pressed
	 * while within the bounds of this GuiComponent.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the mouse button is released while
	 * within the bounds of this GuiComponent.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the mouse wheel is scrolled while
	 * within the bounds of this GuiComponent.
	 *
	 * @param callback the callback
	 */
	...
	/**
	 * Add a callback that is being executed if the text on this GuiComponent
	 * changes.
	 *
	 * @param cons the cons
	 */
	...
	/**
	 * Prepare the GuiComponent and all its child Components (Makes the GuiComponent
	 * visible and adds mouse listeners.). This is, for example, done right before
	...
	public RectangularShape getShape() {
		...
		return new RoundRectangle2D.Double(this.getX(), this.getY(), this.getWidth(), this.getHeight(),
				this.getCurrentAppearance().getBorderRadius(), this.getCurrentAppearance().getBorderRadius());
	}
	...
	/**
	 * Sets the margin size between the GuiComponent's left and right border and the
	 * Text bounds.
	 *
	 ...
	 *
	 * @param textAlignment the new text align
	 */
	 ...
	 *
	 * @param drawTextShadow the boolean determining if a text shadow should be
	 *                       drawn
	 */
	...
	/**
	 * Suspend the GuiComponent and all its child Components (Makes the GuiComponent
	 * invisible and removes mouse listeners.).
	 *
	 */
	public void suspend() {
	 ...
	 *
	 * @param g the <code>Graphics2D</code> object used for drawing
	 */
	private void renderText(final Graphics2D g) {
		...
		if (this.getTextAngle() == 0) {
			if (this.drawTextShadow()) {
				TextRenderer.renderWithOutline(g, this.getTextToRender(g), this.getX() + this.getTextX(),
						this.getY() + this.getTextY(), this.getTextShadowColor(), antialiasing);
			} else {
				TextRenderer.render(g, this.getTextToRender(g), this.getX() + this.getTextX(),
						this.getY() + this.getTextY(), antialiasing);
			}
		} else if (this.getTextAngle() == 90) {
			TextRenderer.renderRotated(g, this.getTextToRender(g), this.getX() + this.getTextX(),
					this.getY() + this.getTextY() - fm.stringWidth(this.getTextToRender(g)), this.getTextAngle(),
					antialiasing);
		} else {
			TextRenderer.renderRotated(g, this.getTextToRender(g), this.getX() + this.getTextX(),
					this.getY() + this.getTextY(), this.getTextAngle(), antialiasing);
		}
	}
}